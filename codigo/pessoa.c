...
static void _showPessoa_paginacao(void *d) {
    Pessoa *p = (Pessoa*)d;
    imprimirPessoa(p);
    printf("\n");
}

void imprimirListaPessoas_paginado(ListaPessoas *l, int itemsPerPage) {
    if (!l || itemsPerPage <= 0) return;

    int count = 0;
    ItemPessoa *it = l->inicio;
    while (it) { count++; it = it->prox; }

    if (count == 0) { printf("Nenhuma pessoa registada.\n"); return; }
    ...
    int idx = 0;
    it = l->inicio;
    while (it) { arr[idx++] = it->pessoa; it = it->prox; }

    paginacao(arr, count, itemsPerPage, _showPessoa_paginacao);
